    ...
    <el-col :span="8" style="padding-right: 10px"
      ...
      <el-card style="margin-top: 20px; height: 460px">
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column
            v-for="(label, prop) in tableLabel"
            :prop="prop"
            :label="label"
            :key="prop"
          />
...
export default {
  ...
  mounted() {
    //调用getData获取homeapi中的data数据
    getData().then(({ data }) => {
      //从data.phdata中取出tableData
      const { tableData } = data.phdata
      ...
      //处理数据
      //xAxis数据
      const { orderData, userData, videoData } = data.phdata //获取orderdata
      // console.log(userData)
      const xAxis = Object.keys(orderData.data[0]) //遍历orderdata.data中的list数组的第一个数组的key值
